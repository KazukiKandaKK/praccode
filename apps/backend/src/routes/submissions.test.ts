...
describe('submissionRoutes', () => {
  ...
  describe('GET /submissions', () => {
    it('正常系: ユーザーのサブミッション一覧と統計情報を返す', async () => {
        mockPrisma.submission.findMany.mockResolvedValue([
            {
                id: 'sub-1', status: 'EVALUATED', createdAt: new Date(), updatedAt: new Date(),
                exercise: { id: 'ex-1', title: 'Ex 1', language: 'ts', difficulty: 1, genre: 'test'},
                answers: [{ score: 80, level: 'B' }, { score: 90, level: 'A' }]
            }
        ]);
  ...
  describe('PUT /submissions/:id/answers', () => {
    ...
    it('正常系: 回答を更新する', async () => {
        mockPrisma.submission.findUnique.mockResolvedValue({ id: submissionId, status: 'DRAFT', answers: [] });
        ...
        mockPrisma.submission.findUnique.mockResolvedValueOnce({ id: submissionId, status: 'DRAFT', answers: [{...answersPayload.answers[0]}]});
  ...
  describe('POST /submissions/:id/evaluate', () => {
    ...
    it('正常系: 評価をキューに入れ、バックグラウンドで実行する', async () => {
        ...
        mockPrisma.submission.findUnique.mockResolvedValue(mockSubmission);
        mockEvaluator.evaluateAnswer.mockResolvedValue({ score: 95, level: 'A', feedback: 'Good', aspects: {} });
    ...
    it('異常系: 評価済みのサブミッションはキューに入れない', async () => {
        ...
        const response = await app.inject({ method: 'POST', url: `/submissions/${submissionId}/evaluate` });
