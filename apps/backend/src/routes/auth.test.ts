...
describe('authRoutes', () => {
  ...
  describe('POST /auth/register', () => {
    ...
    it('正常系: 登録に成功し、確認メールを送信する', async () => {
        ...
        // Verify mocks
        expect(mockPrisma.user.findUnique).toHaveBeenCalledWith({ where: { email: registerPayload.email } });
  ...
  describe('POST /auth/verify-email', () => {
    ...
    it('正常系: 有効なトークンでメール認証に成功する', async () => {
        ...
        // Verify mocks
        expect(mockPrisma.emailVerificationToken.findUnique).toHaveBeenCalledWith({ where: { tokenHash: hashedValidToken }, include: { user: true } });
        expect(mockPrisma.$transaction).toHaveBeenCalled();
        expect(mockPrisma.user.update).toHaveBeenCalledWith({ where: { id: mockVerificationToken.userId }, data: { emailVerified: expect.any(Date) } });
        expect(mockPrisma.emailVerificationToken.delete).toHaveBeenCalledWith({ where: { id: mockVerificationToken.id } });
        expect(mockMail.sendWelcomeEmail).toHaveBeenCalledWith(mockVerificationToken.user.email, mockVerificationToken.user.name);
    });
    ...
    it('異常系: 期限切れのトークンの場合410を返す', async () => {
        ...
        expect(mockPrisma.emailVerificationToken.delete).toHaveBeenCalledWith({ where: { id: expiredToken.id } });
    });

    it('準正常系: 既に認証済みの場合メッセージを返す', async () => {
        const alreadyVerifiedToken = { ...mockVerificationToken, user: { ...mockVerificationToken.user, emailVerified: new Date() }};
        ...
        expect(response.statusCode).toBe(200);
        expect(JSON.parse(response.payload)).toEqual({ message: 'Email already verified. You can now log in.', alreadyVerified: true });
        expect(mockPrisma.emailVerificationToken.delete).toHaveBeenCalledWith({ where: { id: alreadyVerifiedToken.id } });
    });
  });
  ...
  describe('POST /auth/forgot-password', () => {
    ...
    it('正常系: ユーザーが見つかり、リセットメールが送信される', async () => {
        ...
        expect(response.statusCode).toBe(200);
        expect(mockPrisma.passwordResetToken.deleteMany).toHaveBeenCalledWith({ where: { userId: mockUser.id } });
  ...
  describe('POST /auth/reset-password', () => {
    ...
    it('正常系: 有効なトークンでパスワードリセットに成功する', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(JSON.parse(response.payload).message).toContain('Password has been reset successfully');
      ...
      expect(mockPrisma.passwordResetToken.delete).toHaveBeenCalledWith({ where: { id: mockResetToken.id } });
    });
    ...
    it('異常系: 期限切れのトークンの場合410を返す', async () => {
        ...
        expect(response.statusCode).toBe(410);
        expect(JSON.parse(response.payload)).toEqual({ error: 'Token expired' });
        expect(mockPrisma.passwordResetToken.delete).toHaveBeenCalledWith({ where: { id: expiredToken.id } });
    });
