...
describe('GeminiProvider', () => {
    ...
    describe('generate', () => {
        ...
        it('正常系: 正常なレスポンスを返す', async () => {
            ...
            expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/models/gemini-2.5-flash-lite:streamGenerateContent'), expect.any(Object));
        });
...
{"candidates": [{"content": {"parts": [{"text": "World"}]}}]}`;
        ...
        it('異常系: APIキーがない場合エラーを投げる', async () => {
            ...
            await expect(providerWithoutKey.generate(prompt)).rejects.toThrow('GEMINI_API_KEY environment variable is not set');
        });
